...
namespace Opencart\Catalog\Controller\Extension\TelrOc\Payment;

class Telr extends \Opencart\System\Engine\Controller {
	...
	public function index() {
		...
		// Manage the respose url version wise
		if(in_array(VERSION,array('4.0.0.0','4.0.1.0','4.0.1.1'))){
			$return_auth = 'extension/telr_oc/payment/telr|callback';
			$return_decl = 'extension/telr_oc/payment/telr|callback';
			$ivp_update_url = 'extension/telr_oc/payment/telr|ivpcallback';
		}else{
			$return_auth = 'extension/telr_oc/payment/telr.callback';
			$return_decl = 'extension/telr_oc/payment/telr.callback';
			$ivp_update_url = 'extension/telr_oc/payment/telr.ivpcallback';
		...
		$post_data = Array(
			...
			'return_auth'		=> 	$this->url->link($return_auth, '', true),
			'return_can'		=>  $this->url->link('checkout/checkout', '', true),
			'return_decl'		=>  $this->url->link($return_decl, '', true),
			'ivp_update_url'	=>  $this->url->link($ivp_update_url, array('cart_id' => $order_id), true),
		);
		...
		if ($framed) {
			return $this->load->view('extension/telr_oc/payment/telr_frame',$data);
		} else {
			return $this->load->view('extension/telr_oc/payment/telr',$data);
		}
	}
